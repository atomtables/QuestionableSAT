            ...
            if (page.url.pathname === "/questiongets/justone") {
                ...
                params.params = val = JSON.stringify({
                    ...
                    domain: Object.values(lookup.lookupData.domain)
                        [lookup.lookupData.test[parseInt(JSON.parse(page.url.searchParams.get("jsonparams"))["test"]) - 1].id - 1].map((v) => v.primaryClassCd)
                        .join(","),
                });
            } else {
                params.params = val = JSON.stringify({
                    ...
                    domain: Object.values(lookup.lookupData.domain)
                        [lookup.lookupData.test[parseInt(selectedDetails.section)].id - 1].map((v) => v.primaryClassCd)
                        .join(","),
                });
            }
