<script lang="ts">
    ...
    let currentQuestion: [
        Question,
        QuestionDetail,
        { selected: string; review: boolean },
    ] = $derived(
        currentQuestionIndex !== -1
            ? currentQuestionIndex < questions.length
                ? questions[currentQuestionIndex]
                : null
            : [null, null, { selected: null, review: false }],
    );
    ...
    function getRandomFromArray<T>(arr: Array<T>) {
        if (arr.length === 0) {
            alert(
                "The question bank is out of options.",
                "Please lessen your filters and try again.",
            ).then(() => goto("/topics"));
        }
    ...
    const appliedFilters = (question: Question) => {
        ...
        for (let topic of lookupData)
            for (let skill of topic.skill) skillsInSelectedSection.push(skill);
        let selectedSubtopics = [];
        for (let [id, selected] of Object.entries(selectedDetails.subtopics))
            if (selected) selectedSubtopics.push(id);
        skillsInSelectedSection = skillsInSelectedSection.filter((v) =>
            selectedSubtopics.includes(v.id),
        );

        let classesInSelectedSection = [];
        for (let topic of lookupData) classesInSelectedSection.push(topic);
        let selectedClasses = [];
        for (let [id, selected] of Object.entries(selectedDetails.topics))
            if (selected) selectedClasses.push(id);
        classesInSelectedSection = classesInSelectedSection.filter((v) =>
            selectedClasses.some((x) => x === v.id),
        );

        return (
            classesInSelectedSection.some(
                (v) => v.primaryClassCd === question.primary_class_cd,
            ) &&
            skillsInSelectedSection.some(
    ...
    async function getQuestion(
    ...
    ): Promise<[Question, QuestionDetail]> {
        ...
        try {
            ...
            seenInSessions = [];
            console.warn(
                "seenInSessions wasn't set to a value parsable by JSON... resetting.",
            );
        ...
        if (bank.length < 1) {
            await alert(
                "Out of questions",
                "You have reached the end of your selected questions.",
            );
            return;
        }
        ...
        for (let [dat, question, { selected, review }] of questions) {
            ...
            currentQuestionHistory.push([
                ...
                dat,
                {
                    correct:
                        question.type === "mcq"
                            ? question.keys.includes(
                                  question.answerOptions[selected]?.id || null,
                              )
                            : question.keys.includes(selected),
                    selected:
                        question.type === "mcq"
                            ? question.answerOptions[selected]?.id
                            : selected,
                    reviewed: review,
                },
            ]);
    ...
    let currentlyReviewed = $derived(
        currentlyReviewing !== null && currentlyReviewing + 1,
    );
...
{#if timetogo}
    {#if currentlyReviewing !== null}
        <div class="z-10000" transition:slide|global>
            <Bluebook
                ...
                bind:questionOutside={
                    currentQuestionHistory[currentlyReviewing][1]
                }
    ...
    {:else}
        <div
            class="flex flex-col w-screen h-screen items-center justify-center bg-neutral-900/50 backdrop-blur-sm p-10 bg-opacity-50 text-white min-h-screen"
        >
            <div class="max-w-3xl mx-auto w-full space-y-8">
                <div class="flex flex-col items-center gap-5">
                    ...
                    <div class="flex justify-between items-center w-full">
                        <div class="flex-1 text-center">
                            <div class="text-xl font-bold opacity-50">
                                1. Exam
                            </div>
                        </div>
                        <div class="flex-1 text-center">
                ...
                >
                    ...
                    <div class="text-gray-300">
                        You answered <b
                            >{currentQuestionHistory.length} questions</b
                        >
                        and got
                        <b
                            >{currentQuestionHistory.filter((v) => v[2].correct)
                                .length} correct</b
                        >
                    </div>
                        ...
                        <table class="pt-4 w-full">
                            ...
                            <tbody>
                                {#each currentQuestionHistory as [_, data, { correct, reviewed }], i}
                                    <tr
                                        class="hover:bg-blue-800/50 active:bg-blue-700/50 transition-colors cursor-pointer"
                                        onclick={() => (currentlyReviewing = i)}
                                        aria-roledescription="Review question"
                                    >
                                        <td>{i + 1}</td>
    ...
    <div transition:slide|global class="z-10000">
        ...
        >
            {#snippet overviewSnippet()}
                <div
                    class="max-w-4xl flex flex-col items-center justify-center m-auto"
                >
                    ...
                    <div class="grid grid-cols-8">
                        {#each questions as [_, __, { selected, review }], i}
                            <button
                                ...
                                class="relative cursor-pointer w-14 h-14 text-3xl font-bold font-sans flex items-center justify-center {isNaN(
                                    parseInt(selected),
                                )
                                    ? 'border-dashed border-2 border-black text-black hover:bg-blue-100'
                                    : 'bg-blue-900 text-white'}  m-3"
                            >
                                {i + 1}
                                {#if review}
                                    <img
                                        src={bookmarked}
                                        alt="bookmarked"
                                        class="bg-white h-6 absolute -right-2 -top-2"
                                    />
                                {/if}
                            </button>
                        {/each}
            ...
            {#snippet miniOverviewSnippet()}
                ...
                <div class="grid grid-cols-8 w-full gap-2 p-5">
                    {#each questions as [_, __, { selected, review }], i}
                        <button
                            onclick={() => (currentQuestionIndex = i)}
                            class="relative cursor-pointer w-8 h-8 text-base font-bold font-sans flex items-center justify-center {currentQuestionIndex ===
                                i && 'bg-blue-200'} {isNaN(parseInt(selected))
                                ? 'border-dashed border-2 border-black text-blue-900 hover:bg-blue-100'
                        ...
                        >
                            ...
                            {#if review}
                                <img
                                    src={bookmarked}
                                    alt="bookmarked"
                                    class="bg-white h-4 absolute -right-1 -top-1"
                                />
                            {/if}
                        </button>
                    {/each}
                ...
                <div class="pb-5">
                    <Button
                        resetStyling
                        class="border-blue-700 border-2 text-blue-700 hover:text-white hover:bg-blue-600 active:bg-blue-500 cursor-pointer transition-colors font-bold px-3 rounded-full py-1"
