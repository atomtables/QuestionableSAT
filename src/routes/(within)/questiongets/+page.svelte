...
<div class="flex flex-col w-screen h-screen items-center justify-center bg-neutral-900/50 backdrop-blur-sm p-10 bg-opacity-50 text-white min-h-screen">
    <div class="max-w-3xl mx-auto w-full space-y-8">
        ...
        <div class="space-y-2 bg-gray-800 rounded-lg p-6 shadow-md" transition:slide={{ duration: 50 }}>
            ...
            {:then questions}
                ...
                <div class="text-gray-300">
                    You have {filtered.length} questions available to go through right now, out of which
                    <button class="underline cursor-pointer" onclick={async () => (openResetDialog = true)}>you have viewed {viewedQuestions.filter((v) => questions.some((x) => x.external_id === v || x.ibn === v)).length + "."} </button>
                </div>
                <div class="pt-0">Preparation doesn't just have to be exactly like a test. You should choose the best way to prepare that'll help you master the skill you're having trouble with.</div>
                ...
                <div class="flex flex-row justify-center items-center px-1 gap-2">
                    <Input type="radio" radioGroup="testType" radioId={3} class="w-min" bind:value />
                    <div class="w-full">Answer random questions from your set, testing a variety of difficulties.</div>
                </div>
                {#if value === -1}
                    <div class="flex w-full flex-col" transition:slide>
                        <div class="flex flex-row justify-center w-full items-center -my-1">
                            <Input bind:value={startDiff} name="Starting Difficulty" type="dropdown" elements={["Easy", "Medium", "Hard"]} className="!w-full" />
                        </div>
                        <div class="flex flex-col justify-center -my-1">
                            <Input bind:value={answerNumber} name="Maximum streak before finishing session" type="text" className="!w-full" />
                            {#if !lookAt}
                                <div class="text-red-500 pl-1 -mt-2">Must be a number</div>
                            {/if}
                        </div>
                        <div class="flex flex-row justify-center items-center -my-1">
                            <Input bind:value={maxTries} name="Maximum tries to answer a question before showing the answer" type="dropdown" elements={["None", 1, 2, 3]} className="!w-full" />
                        </div>
                        <div class="flex flex-row justify-center items-center px-1 gap-2">
                            <Input bind:value={hideQuestions} type="checkbox" class="flex flex-row w-min" />
                ...
                {:else if value === 1}
                    <div class="flex flex-col" transition:slide>
                        <div class="flex flex-col justify-center -my-1">
                            <Input bind:value={maxQuestions} name="Amount of questions" type="text" className="!w-full" />
                            {#if !lookAtQ}
                                <div class="text-red-500 pl-1 -mt-2">Must be a number</div>
                            {/if}
                        </div>
                        <div class="flex flex-col justify-center -my-1">
                            <Input bind:value={time} name="Minutes the test should take" type="text" className="!w-full" />
                            {#if !lookAtT}
                                <div class="text-red-500 pl-1 -mt-2">Must be a number</div>
                            {/if}
                        </div>
                        <div class="flex flex-row justify-center items-center -my-1">
                            <Input bind:value={startDiff} name="Difficulty Skew" type="dropdown" elements={["Easy", "Medium", "Hard"]} className="!w-full" title="Choose whether the bell curve of difficulty should be skewed. Medium is the best option here." />
                        </div>
                        <div class="flex flex-row justify-center items-center px-1 gap-2">
                            <Input bind:value={hideQuestions} type="checkbox" class="flex flex-row w-min" />
                            <div class="w-full">Hide questions that have been seen in other sessions.</div>
        ...
        <div class="flex justify-between items-center mt-6">
            ...
            <Button disabled={!lookAt} onclick={() => continu()}>I'm ready</Button>
        </div>
    </div>
</div>
...
>
    <div>QuestionableSAT keeps track of the questions you view, so you can practice with unique questions each time. You can disable this setting, or reset the questions you've seen if you want to start with a blank slate.</div>
    <div class="py-2">
        <Button onclick={() => (localStorage.setItem("seen", JSON.stringify([])), (viewedQuestions = []))}>Reset all seen questions</Button>
    </div>
</Dialog>
