<script lang="ts">
    ...
    const lookAt = $derived(
        answerNumber === null || !isNaN(parseInt(answerNumber)),
    );
    ...
    const lookAtQ = $derived(
        maxQuestions === null || !isNaN(parseInt(maxQuestions)),
    );
    let time = $state("30");
    ...
    const continu = async () => {
        if (value === -1)
            await goto(
                `/questiongets/khanstyle?start=${["e", "m", "h"][startDiff]}&tries=${maxTries}&ignoreViewed=${hideQuestions}&streak=${answerNumber}`,
            );
        else if (value === 1) {
            console.log(time, parseInt(time), parseInt(time) * 60);
            await goto(
                `/questiongets/practicetest?questions=${parseInt(maxQuestions)}&time=${parseInt(time) * 60}&ignoreViewed=${hideQuestions}&diff=${["E", "M", "H"][startDiff]}`,
            );
        } else if (value === 3) {
            await goto(
                `/questiongets/random?tries=${maxTries}&ignoreViewed=${hideQuestions}&streak=${answerNumber}`,
            );
        }
    };
</script>
...
>
    <div class="max-w-3xl mx-auto w-full space-y-8">
        ...
        >
            ...
            {:then questions}
                ...
                <div class="text-gray-300">
                    You have {filtered.length} questions available to go through
                    right now, out of which
                    <button
                        class="underline cursor-pointer"
                        onclick={async () => (openResetDialog = true)}
                        >you have viewed {viewedQuestions.filter((v) =>
                            questions.some(
                                (x) => x.external_id === v || x.ibn === v,
                            ),
                        ).length + "."}
                    </button>
                </div>
                <div class="pt-0">
                    Preparation doesn't just have to be exactly like a test. You
                    should choose the best way to prepare that'll help you
                    master the skill you're having trouble with.
                </div>
                <div
                    class="flex flex-row justify-center items-center px-1 gap-2"
                >
                    <Input
                        type="radio"
                        radioGroup="testType"
                    ...
                    />
                    <div class="w-full">
                        Answer questions that get progressively harder as you
                        answer correctly.
                    </div>
                </div>
                <div
                ...
                >
                    ...
                    />
                    <div class="w-full">
                        Answer a fixed amount of questions similar to (but not)
                        a practice test.
                    </div>
                </div>
                <div
                ...
                >
                    ...
                    />
                    <div class="w-full">
                        Answer random questions from your set, testing a variety
                        of difficulties.
                    </div>
                </div>
                {#if value === -1}
                    <div class="flex flex-col" transition:slide>
                        ...
                        >
                            ...
                            />
                            <div class="w-full">
                                Hide questions that have been seen in other
                                sessions.
                            </div>
                        </div>
                    </div>
        ...
        <div class="flex justify-between items-center mt-6">
            <Button onclick={() => goto(`/topics?left=3`)}>Previous</Button>

            <Button disabled={!lookAt} onclick={() => continu()}>
                I'm ready
            </Button>
        </div>
    </div>
...
<Dialog
    ...
    title="Your seen questions"
    description="You have seen {viewedQuestions.length} questions in total."
>
    <div>
        QuestionableSAT keeps track of the questions you view, so you can
        practice with unique questions each time. You can disable this setting,
        or reset the questions you've seen if you want to start with a blank
        slate.
    </div>
    <div class="py-2">
        <Button
