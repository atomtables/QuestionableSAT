...
<script lang="ts">
    ...
    onMount(async () => {
        ...
        if (!questionID) {
            goto("/topics").then(() => {
                alert(
                    "Error",
                    "This question link is invalid. Please request the person who sent you the link to send you a valid link.",
                );
            });
        }
        try {
            ...
            console.log(qs);
            currentQuestionOutside = qs.find(
                (q) =>
                    q.external_id === questionID
            );
    ...
    let currentlyTimered = $derived(
        `${Math.floor(currentQuestionHistory[currentlyReviewing][2].timeInt / 60)}:${currentQuestionHistory[currentlyReviewing][2].timeInt % 60 < 10 ? "0" + (currentQuestionHistory[currentlyReviewing][2].timeInt % 60).toString() : (currentQuestionHistory[currentlyReviewing][2].timeInt % 60).toString()}`,
    );
