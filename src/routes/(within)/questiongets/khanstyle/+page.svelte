<script lang="ts">
    ...
    async function submitHandler(): Promise<[boolean, number]> {
        ...
        currentQuestionHistory.push([
            currentQuestion,
            currentQuestionOutside,
            {
                correct: currentQuestion.type === "mcq" ? currentQuestion.keys.includes(currentQuestion.answerOptions[selectedOption].id) : currentQuestion.keys.includes(selectedOption.toString()),
                selected: currentQuestion.type === "mcq" ? currentQuestion.answerOptions[selectedOption].id : selectedOption.toString(),
                timeInt: timerInt,
            },
        ]);
        ...
        if (currentQuestion.type === "mcq" ? currentQuestion.keys.includes(currentQuestion.answerOptions[selectedOption].id) : currentQuestion.keys.includes(selectedOption.toString())) {
            ...
            currentStreak += 1;
            if (maxStreak && currentStreak >= maxStreak) return [true, -1];
            return [true, 0];
        } else {
            if (!decreased) {
                currentScoreTarget -= scoreTargetIncrease;
    ...
    let statusThing = $derived(`Difficulty between ${(currentScoreTarget - 1).toFixed(2)} to ${(currentScoreTarget + 1).toFixed(2)}`);
</script>
