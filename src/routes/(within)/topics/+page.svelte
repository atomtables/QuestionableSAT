<script lang="ts">
    ...
    let { data }: { data: { lookup: LookupData } } = $props(); // assumes lookup.lookupData etc.
...
<div class="flex flex-col w-screen h-screen items-center justify-center bg-neutral-900/50 backdrop-blur-sm p-10 bg-opacity-50 text-white min-h-screen" in:fade={{ easing: cubicIn, duration: 200 }}>
    <div class="max-w-3xl mx-auto w-full space-y-8">
            ...
            <div class="space-y-4 bg-gray-800 rounded-lg p-6 shadow-md" transition:slide>
                ...
                {#each lookup.lookupData.assessment as { id, text }, i}
                    <div class="flex items-center gap-3">
                        <Input type="radio" radioGroup="assessment" radioId={i} bind:value={setup.test} class="w-min" />
        ...
        {#if currentStep === 2}
            <div class="space-y-4 bg-gray-800 rounded-lg p-6 shadow-md" transition:slide>
                ...
                {#each lookup.lookupData.test as { id, text }, i}
                    <div class="flex items-center gap-3">
                        <Input type="radio" radioGroup="section" radioId={i} bind:value={setup.section} class="w-min accent-indigo-500" />
                        <span class="text-lg">{text}</span>
                    </div>
                {/each}
                <a href="/everything" class="text-gray-300 hover:underline text-sm">Just show me everything.</a>
            ...
            <div class="space-y-4 bg-gray-800 rounded-lg p-6 shadow-md" transition:slide>
                ...
                {#if typeof setup.section === "number"}
                    <div class="max-h-60 overflow-y-auto space-y-3 mt-4">
                        ...
                        {#each Object.values(lookup.lookupData.domain)[setup.section] as topic}
                            <div class="flex flex-col space-y-2">
                                {#if topic.skill}
                                    <Collapsible title={topic.text} description={`${topic.skill.filter((s) => setup.subtopics[s.id]).length}/${topic.skill.length} selected`}>
                                        {#snippet shelf()}
                                            <Button
                                                transparent
                                                class="-mt-2.5"
                                        ...
                                        {#each topic.skill as skill}
                                            <div class="flex items-center gap-3 ml-4">
                                                <Input type="checkbox" bind:value={setup.subtopics[skill.id]} class="w-min accent-indigo-500" />
                    ...
                    <div class="flex items-center gap-3 mt-4">
                        <Input type="checkbox" bind:value={setup.ignoreLive} class="w-min accent-indigo-500" />
                ...
                <div>
                    ...
                    <ul class="list-disc list-inside ml-4">
                        {#each Object.entries(setup.topics).filter(([_, v]) => v) as [topicId, _]}
                            {""}
                            {@const topic = Object.values(lookup.lookupData.domain)[setup.section].find((t) => parseInt(t.id.toString()) === parseInt(topicId))}
                            <li>
                                {topic.text} ({topic.skill.filter((s) => setup.subtopics[s.id]).length}/{topic.skill.length} selected)
                            </li>
                        {/each}
                    </ul>
                </div>
        ...
        <div class="flex justify-between items-center mt-6">
            ...
            {#if currentStep < 4}
                <Button onclick={goNext} disabled={verifyNext}>Next</Button>
            {:else}
